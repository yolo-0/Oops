...
async def lifespan(app: FastAPI):
    ...
    from mcp.business_tools import query_order, query_logistics
    ...
    async def query_order_handler(params, context=None):
        return await query_order(params.get("order_id", ""))

    async def query_logistics_handler(params, context=None):
        return await query_logistics(params.get("order_id", ""))
    ...
    _tool_manager.register(Tool(
        ...
        description="查询物流详情",
        handler=query_logistics_handler,
        schema={
            "type": "object",
            "properties": {"order_id": {"type": "string"}},
        },
    ...
    logger.info("Oops! 已就绪")
    yield

    await _monitor.stop()
    if _memory is not None:
        await _memory.close()
    logger.info("Oops! 已关闭")
...
async def _build_business_context(message: str, intent, entities: dict) -> tuple[str, bool]:
    """根据意图调用对应的业务工具，并将结果拼接成上下文。"""
    if _tool_manager is None:
        return "", False
    ...
    if intent == IntentCategory.ORDER_STATUS:
        ...
        if not order_id:
            return "无法查询订单：缺少订单号。", False
        res = await _tool_manager.call("query_order", {"order_id": order_id})
        if res.success:
            return f"[业务系统返回 - 查询订单]\n{json.dumps(res.data, ensure_ascii=False)}", True
            
    elif intent == IntentCategory.LOGISTICS:
        order_list = entities.get("order_id", [])
        order_id = order_list[0] if order_list else ""
        if not order_id:
            return "无法查询物流：缺少订单号。", False
        res = await _tool_manager.call("query_logistics", {"order_id": order_id})
        if res.success:
            return f"[业务系统返回 - 查询物流]\n{json.dumps(res.data, ensure_ascii=False)}", True
...
async def run_eval(body: Optional[EvalRunInput] = None):
    ...
    if body and body.intent_cases is not None:
        if len(body.intent_cases) == 1 and body.intent_cases[0].message == "string":
    ...
    if body and body.dialog_cases is not None:
        if len(body.dialog_cases) == 1 and body.dialog_cases[0].question == "string":
    ...
    if _memory is not None:
        try:
            mem_eval = MemoryEvaluator(_memory)
            memory_results = await mem_eval.run_all_tests()
        except Exception as e:
            logger.error(f"Memory Evaluator 运行失败: {e}")
