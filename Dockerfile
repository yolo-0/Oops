...
RUN useradd -m -u 1000 oops

# 从依赖阶段复制已安装的包
COPY --from=dependencies /usr/local/lib/python3.12/site-packages /usr/local/lib/python3.12/site-packages
COPY --from=dependencies /usr/local/bin /usr/local/bin
# 复制预下载的 ONNX 模型缓存
COPY --from=dependencies --chown=oops:oops /root/.cache/chroma /home/oops/.cache/chroma

# 复制应用代码
COPY --chown=oops:oops . .

# 创建必要目录，只调整运行期需要写入的目录权限，避免递归 chown 整个应用。
RUN mkdir -p /app/data/chroma /app/logs /app/config && \
    chown oops:oops /app/data /app/data/chroma /app/logs /app/config
USER oops
