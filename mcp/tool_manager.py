...
class SubQueries(BaseModel):
    queries: List[str] = Field(description="不同角度的搜索子查询")

class RerankOrder(BaseModel):
    order: List[int] = Field(description="按相关性降序排列的索引列表")
...
"""
    ...
    async def search_with_rewrite(
        ...
        logger.info(f"合并去重: {len(merged)} 条")
    ...
    async def _rerank(self, query: str, items: List[Any], top_k: int) -> List[Any]:
        ...
        prompt = f"""根据用户查询，对以下检索结果按相关性打分（0-10）。
用户查询: "{query}"
检索结果:
{items_text}
"""
        ...
        try:
            ...
            logger.info(f"已筛选出最相关的{min(len(items), top_k)}条")
