...
class PerformanceMonitor:
    ...
    THRESHOLDS = {
        ...
        "agent_avg_ms":        (10000, Severity.WARNING, "greater_than"), # LLM生成完整长文本通常需要 4-8 秒，放宽至 10 秒
    ...
    def _routing_penalty(success_rate: float, avg_ms: float) -> float:
        ...
        if avg_ms > 10000:
            penalty += min(0.4, (avg_ms - 10000) / 10000)
    ...
    def _check_threshold(self, metric: str, value: float, label: str) -> None:
        ...
        if triggered:
            ...
            for a in self._alerts:
                if a.metric == alert_metric and not a.resolved:
                    a.resolved = True
                    logger.info(f"[RESOLVED] {alert_metric} 恢复正常 (当前值: {value:.3f})")
