...
**解决方案**： 
* **实时同步**：构建实时知识库同步机制，保障数据新鲜度
* **检索优化**：引入查询改写（Query Rewrite）与 LLM 重排（Rerank）
* **拒答策略**：引入可信度评分机制，置信度低于阈值时自动触发拒答
* **兜底追问**：意图识别层配备"兜底追问"策略，规避歧义输入
* **槽点提取**：提取关键槽点并存储到memory中，避免噪声持续注入 
...
**解决方案**：
* **异步并行**：在业务主轴上将"意图识别"与"知识库检索"链路解耦并行；底层子任务中，意图识别内的 Embedding 计算与 LLM 推理以及RAG 的多路子查询都通过异步并发调度，整体延迟降低 20% 以上。
* **熔断降级**：知识库检索超过 10 秒即触发单次降级兜底；连续超时 5 次直接熔断全局断路器，有效保护后端线程池不被压垮。
...
**解决方案**：
* **语义缓存**：接入 ChromaDB 语义缓存，可降低 70% 的重复查询 Token 消耗
* **上下文压缩**：多轮对话上下文压缩——将历史消息提炼为"用户画像（偏好与习惯）"及"服务总结"存档，大幅缩减 Prompt 长度
